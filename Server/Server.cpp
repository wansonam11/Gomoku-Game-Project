	...
	while (true) {
		...
		if ((size = recv(client->getClientSocket(), received, 256, NULL)) > 0) {
			...
			if (receivedString.find("[Enter]") != -1) {
				...
				//2명이상이 동일한 방에 들어가 있는 경우
				if (clientCount >= 2) {
					fullClient(client);
				}
				//접속 성공
				client->setRoomID(roomInt);
				cout << "클라이언트 [" << client->getClientID() << "]: " << client->getClientID;
				//방에 성공적으로 접속했다고 메세지 전송

				enterClient(client);
				//상대방이 이미 방에 들어가 있는 경우 게임시작
				if (clientCount == 1) {
					playClient(roomInt);
				}
			}
			...
			cout << "클라이언트[" << client->getClientID << "]의 연결이 끊어졌습니다." << client->getClientID;
			//게임에서 나간 플레이어 찾기
			for (int i = 0; i < connections.size(); i++) {
